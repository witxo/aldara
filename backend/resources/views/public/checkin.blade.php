        ...
        <form method="POST" action="{{ route('public.checkin.submit', $reservation->checkin_token) }}" class="space-y-6" novalidate>
            ...
            <div x-show="currentGuest === {{ $i }} && {{ $i }} < totalGuests" x-cloak>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div x-show="editableChildren > 0">
                        <label class="block text-sm font-medium text-gray-700" for="g-parentesco-{{ $i }}">Parentesco <span class="text-xs text-gray-400">(rellenar si hay menores)</span></label>
                        <input type="text" name="guests[{{ $i }}][parentesco]" id="g-parentesco-{{ $i }}" maxlength="5" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="H, M, P...">
                    </div>
                    <div>
